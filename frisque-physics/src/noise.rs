/// Simple Perlin noise implementation for wind turbulence.
/// Deterministic permutation table for replay consistency.

const PERM: [u8; 256] = [
    151,160,137,91,90,15,131,13,201,95,96,53,194,233,7,225,
    140,36,103,30,69,142,8,99,37,240,21,10,23,190,6,148,
    247,120,234,75,0,26,197,62,94,252,219,203,117,35,11,32,
    57,177,33,88,237,149,56,87,174,20,125,136,171,168,68,175,
    74,165,71,134,139,48,27,166,77,146,158,231,83,111,229,122,
    60,211,133,230,220,105,92,41,55,46,245,40,244,102,143,54,
    65,25,63,161,1,216,80,73,209,76,132,187,208,89,18,169,
    200,196,135,130,116,188,159,86,164,100,109,198,173,186,3,64,
    52,217,226,250,124,123,5,202,38,147,118,126,255,82,85,212,
    207,206,59,227,47,16,58,17,182,189,28,42,223,183,170,213,
    119,248,152,2,44,154,163,70,221,153,101,155,167,43,172,9,
    129,22,39,253,19,98,108,110,79,113,224,232,178,185,112,104,
    218,246,97,228,251,34,242,193,238,210,144,12,191,179,162,241,
    81,51,145,235,249,14,239,107,49,192,214,31,181,199,106,157,
    184,84,204,176,115,121,50,45,127,4,150,254,138,236,205,93,
    222,114,67,29,24,72,243,141,128,195,78,66,215,61,156,180,
];

fn hash(i: i32) -> u8 {
    PERM[(i & 255) as usize]
}

fn fade(t: f32) -> f32 {
    t * t * t * (t * (t * 6.0 - 15.0) + 10.0)
}

fn lerp(a: f32, b: f32, t: f32) -> f32 {
    a + t * (b - a)
}

fn grad3d(hash: u8, x: f32, y: f32, z: f32) -> f32 {
    let h = hash & 15;
    let u = if h < 8 { x } else { y };
    let v = if h < 4 { y } else if h == 12 || h == 14 { x } else { z };
    (if h & 1 == 0 { u } else { -u }) + (if h & 2 == 0 { v } else { -v })
}

pub fn perlin3d(x: f32, y: f32, z: f32) -> f32 {
    let xi = x.floor() as i32;
    let yi = y.floor() as i32;
    let zi = z.floor() as i32;

    let xf = x - x.floor();
    let yf = y - y.floor();
    let zf = z - z.floor();

    let u = fade(xf);
    let v = fade(yf);
    let w = fade(zf);

    let a  = hash(xi).wrapping_add(hash(yi)) as i32;
    let aa = hash(a).wrapping_add(hash(zi)) as i32;
    let ab = hash(a.wrapping_add(1)).wrapping_add(hash(zi)) as i32;
    let b  = hash(xi.wrapping_add(1)).wrapping_add(hash(yi)) as i32;
    let ba = hash(b).wrapping_add(hash(zi)) as i32;
    let bb = hash(b.wrapping_add(1)).wrapping_add(hash(zi)) as i32;

    lerp(
        lerp(
            lerp(grad3d(hash(aa), xf, yf, zf), grad3d(hash(ba), xf - 1.0, yf, zf), u),
            lerp(grad3d(hash(ab), xf, yf - 1.0, zf), grad3d(hash(bb), xf - 1.0, yf - 1.0, zf), u),
            v,
        ),
        lerp(
            lerp(grad3d(hash(aa.wrapping_add(1)), xf, yf, zf - 1.0), grad3d(hash(ba.wrapping_add(1)), xf - 1.0, yf, zf - 1.0), u),
            lerp(grad3d(hash(ab.wrapping_add(1)), xf, yf - 1.0, zf - 1.0), grad3d(hash(bb.wrapping_add(1)), xf - 1.0, yf - 1.0, zf - 1.0), u),
            v,
        ),
        w,
    )
}

pub fn perlin4d(x: f32, y: f32, z: f32, t: f32, seed: u32) -> f32 {
    // Simple 4D noise by combining two 3D samples offset by seed and time
    let s = seed as f32 * 31.7;
    let n1 = perlin3d(x + s, y + t, z);
    let n2 = perlin3d(x, y + s, z + t);
    (n1 + n2) * 0.5
}
